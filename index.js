...
client.on('change_state', state => {
    ...
    client.on('message', async msg => {
		...
		else if (msg.body.startsWith('!oto') && msg.from == process.env.NOHP + "@c.us") {
            client.sendMessage(msg.from, 'Okee.. 🟢🟢🟢 Otomatis Kirim Laporan, Runing...!');
            console.log('Perintah !oto diterima.');
           // let chat = await msg.getChat();
           // chat.sendSeen();
            intervalID = setInterval(() => {
            manager.query(`select jam,jamterima, idreseller id, namareseller nama, kodeproduk KP, tujuan, namaterminal terminal from transaksi where statustransaksi not in('1','2') and jam < now() - interval 5 minute limit 50`, function (err, rows, fields){
            const fs= require('fs');
            var _0x986d=["\x5B\x0A","\x2C\x0A\x0A","\x6A\x6F\x69\x6E","\x73\x74\x72\x69\x6E\x67\x69\x66\x79","\x6D\x61\x70","\x0A\x5D"];var pending=_0x986d[0]+ rows[_0x986d[4]]((_0xd6d6x2)=>{return JSON[_0x986d[3]](_0xd6d6x2)})[_0x986d[2]](_0x986d[1])+ _0x986d[5]
            client.sendMessage(msg.from, pending);
            console.log('data Trx Pending Terkirim..');
            })
            }, 5 * 60 * 1000);
            }
            else if (msg.body.startsWith('!stop') && msg.from == process.env.NOHP + "@c.us") {
                client.sendMessage(msg.from, 'Okee.. 🔴🔴🔴 STOP Otomatis Kirim Laporan...!');
		...
		else if (msg.body.startsWith('!sendto ') && msg.from == process.env.NOHP+"@c.us") {
		...
		else if (msg.body.startsWith('!pending') && msg.from == process.env.NOHP + "@c.us") {
		let chat = await msg.getChat();
        chat.sendSeen();
